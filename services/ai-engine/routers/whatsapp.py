...
from models.schemas import WhatsAppMessage, WhatsAppSendRequest, ChatSimulateRequest
...
@router.post("/chat")
async def simulate_chat(body: ChatSimulateRequest):
    """
    Simulate the merchant sending a message to the AI bot from the web UI.
    """
    merchant_id = body.merchant_id
    body_text = body.message.strip()
    
    # Store merchant's message as "outbound" so UI shows it on the right
    msg_in = db.insert("whatsapp_messages", {
        "merchant_id": merchant_id,
        "direction": "outbound",
        "recipient_type": "bot",
        "recipient_phone": "simulator",
        "message_type": "text",
        "content": body_text,
        "status": "sent",
    })
    # Intercept demo quick replies
    quick_replies_demo = {
        "GST status": "GST Status:\nMarch 2026 - Ready to File\nDue: 20 April\nTax payable: Rs 44,100\nITC available: Rs 13,200\nNet: Rs 30,900\n\nFile karein?",
        "Remind overdue": "8 overdue customers ko reminder bhej raha hoon...\nDone! 8 reminders sent with payment links.",
        "Customer pulse": "Customer Pulse:\nChampions: 25\nLoyal: 45\nAt Risk: 45 (Alert!)\nChurned: 30\n\nMeena Devi 35 din se nahi aayi - winback bhejein?",
        "Cash forecast": "Next 30 days forecast:\nPredicted Income: Rs 9,20,000\nPredicted Expense: Rs 6,10,000\nNet: Rs 3,10,000\n\nAlert: May first week mein cash crunch possible. Rs 1.2L reserve rakhein."
    }

    if body_text in quick_replies_demo:
        reply_text = quick_replies_demo[body_text]
    else:
        try:
            ...
            reply_text = "Main samajh nahi paaya. Kripya dobara bolein."
        
    # Store bot's reply as "inbound" so UI shows it on the left
    msg_out = db.insert("whatsapp_messages", {
        "merchant_id": merchant_id,
        "direction": "inbound",
    ...
    try:
        ...
        if audio_data_uri and "base64," in audio_data_uri:
            ...
            if len(audio_bytes) > 100:
                ...
                audio_path = f"/tmp/vyapaar_tts_{audio_id}.wav"
...
async def serve_tts_audio(audio_id: str):
    ...
    for ext, mime in [(".wav", "audio/wav"), (".ogg", "audio/ogg"), (".mp3", "audio/mpeg")]:
        audio_path = f"/tmp/vyapaar_tts_{audio_id}{ext}"
