...
async def list_enriched_customers(
    ...
    for c in customers:
        ...
        txn_count = txn_stats.get("count", 0) or c.get("total_visits", 0) or 0
        total_spent = txn_stats.get("total", 0.0) or float(c.get("total_spent", 0) or 0)
...
async def get_at_risk_customers(merchant_id: str):
    ...
    for c in customers:
        last_date_str = c.get("last_visit")
        ...
        days_since = (today - last_date).days
        total_amount = c.get("total_spent", 0)
        total_txns = c.get("total_visits", 0)

        # Estimate average monthly spend (rough)
        avg_monthly = float(total_amount) / max(total_txns, 1) * 4  # assume ~4 visits/month
