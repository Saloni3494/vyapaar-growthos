...
async def recalculate_payscore(merchant_id: str):
    ...
    # Persist
    existing = db.select("payscores", filters={"merchant_id": merchant_id}, single=True)
    payscore_data = {
        "merchant_id": merchant_id,
        "score": result["score"],
        "grade": result["grade"],
    ...
    if existing:
        db.update("payscores", existing["id"], payscore_data)
    else:
        db.insert("payscores", payscore_data)
    ...
    db.insert("payscore_history", {
        ...
        "score": result["score"],
        "grade": result["grade"],
        "feature_breakdown": result["factors"],
