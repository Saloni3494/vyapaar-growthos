...
def _deduct_inventory(merchant_id: str, item_name: str, qty: float, invoice_number: str):
    """If an inventory item matches, deduct stock and log adjustment."""
    try:
        from models.db import get_client as _gc
        from models.db import insert as db_insert
        supa = _gc()
        matches = (
        ...
        if matches:
            item = matches[0]
            old_qty = item.get("current_qty", 0) or 0
            new_qty = max(0, old_qty - qty)
            supa.table("inventory").update({"current_qty": new_qty}).eq("id", item["id"]).execute()
            
            # Log the adjustment
            db_insert("inventory_adjustments", {
                "inventory_id": item["id"],
                "merchant_id": merchant_id,
                "direction": "out",
                "qty": qty,
                "reason": f"Sale via Invoice {invoice_number}",
                "previous_qty": old_qty,
                "new_qty": new_qty,
                "adjusted_at": datetime.now().isoformat()
            })
            
            logger.info("Inventory deducted & logged: %s qty %s -> %s", item["item_name"], old_qty, new_qty)
    except Exception as e:
        logger.warning("Inventory deduction failed for '%s': %s", item_name, e)
...
async def create_invoice(body: CreateInvoiceRequest):
    ...
    for item in body.items:
        ...
        # Deduct inventory
        _deduct_inventory(body.merchant_id, item.name, item.qty, invoice_number)
    ...
    # Handle payment logic based on mode
    payment_mode_lower = body.payment_mode.lower()
    is_credit = payment_mode_lower in ["credit", "udhari"]
    
    amount_paid = 0 if is_credit else total
    ...
    if is_credit:
        ...
        try:
            ...
            logger.warning("Failed to create udhari for invoice %s: %s", invoice_number, e)
    else:
        # Auto-create income transaction for paid amount
        try:
            ...
            logger.warning("Failed to create income transaction for invoice %s: %s", invoice_number, e)
...
async def share_invoice(invoice_id: str, body: ShareInvoiceRequest):
    ...
    phone = body.phone or invoice.get("customer_phone") or "+918261983331"
