...
async def send_briefing(merchant_id: str):
    ...
    msg_body = f"🌅 Good morning! Aaj ki business briefing:\n\n{text}"
    wa_result = await send_whatsapp(to=phone, body=msg_body)
    
    # Store in database as inbound to UI (bot to merchant)
    try:
        db.insert("whatsapp_messages", {
            "merchant_id": merchant_id,
            "direction": "inbound",  # bot -> merchant
        ...
        logger.warning("Failed to store briefing in whatsapp_messages: %s", e)
    ...
    try:
        ...
        if audio_data_uri and audio_data_uri.startswith("data:audio"):
            ...
            with open(f"/tmp/vyapaar_tts_{audio_id}.ogg", "wb") as f:
                f.write(audio_bytes)
