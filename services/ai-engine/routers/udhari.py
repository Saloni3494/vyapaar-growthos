...
async def create_udhari(body: UdhariCreate):
    ...
    # Map API fields to DB schema columns
    data["debtor_name"] = data.pop("customer_name")
    data["debtor_phone"] = data.pop("customer_phone", None)
    ...
    db.sync_customer(body.merchant_id, body.customer_name, phone=body.customer_phone, amount_spent=body.amount)

    await realtime.emit_udhari_created(body.merchant_id, udhari)
    await realtime.emit_dashboard_refresh(body.merchant_id)

    # Map database row back to UdhariResponse schema
    return {
        **udhari,
        "customer_name": udhari.get("debtor_name", ""),
        "customer_phone": udhari.get("debtor_phone"),
        "description": udhari.get("notes"),
        "last_reminded_at": udhari.get("last_reminder_at"),
    }
...
async def settle_udhari(udhari_id: str, body: UdhariSettleRequest):
    ...
    if udhari.get("debtor_name"):
        db.sync_customer(merchant_id, udhari.get("debtor_name"), phone=udhari.get("debtor_phone"), amount_spent=body.amount)
    ...
    return {
        **updated,
        "customer_name": updated.get("debtor_name", ""),
        "customer_phone": updated.get("debtor_phone"),
        "description": updated.get("notes"),
        "last_reminded_at": updated.get("last_reminder_at"),
    }
...
async def send_reminder(udhari_id: str):
    ...
    payment_link = udhari.get("payment_link") or f"https://paytm.me/pay/{merchant_id[:8]}/{remaining:.0f}"
