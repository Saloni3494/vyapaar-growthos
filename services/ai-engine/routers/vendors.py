...
async def list_vendors(merchant_id: str):
    ...
    # Fetch all active payables for the merchant in ONE query (fixing N+1 issue)
    all_payables = (
        db.table("payables")
        .select("vendor_id,remaining,status")
        .eq("merchant_id", merchant_id)
        .in_("status", ["pending", "partial", "overdue"])
        .execute()
    ).data or []

    # Group payables by vendor_id
    payables_by_vendor = {}
    for p in all_payables:
        vid = p.get("vendor_id")
        if not vid:
            continue
        if vid not in payables_by_vendor:
            payables_by_vendor[vid] = []
        payables_by_vendor[vid].append(p)

    for v in vendors:
        vendor_payables = payables_by_vendor.get(v["id"], [])
        v["outstanding"] = sum(p.get("remaining", 0) or 0 for p in vendor_payables)
        v["overdue"] = sum(p.get("remaining", 0) or 0 for p in vendor_payables if p.get("status") == "overdue")
    ...
    for p in upcoming:
        if p.get("days_until", 99) <= 3 and p.get("days_until", 99) >= 0:
            try:
                ...
                asyncio.create_task(send_whatsapp("+918261983331", msg))
            except Exception:
                pass
...
async def pay_payable(payable_id: str, body: PaymentRecord):
    ...
    new_remaining = p["amount"] - new_amount_paid
    
    if new_remaining <= 0:
        new_status = "paid"
        new_remaining = 0
        new_amount_paid = p["amount"] # cap amount paid to payable amount
    else:
        # Preserve overdue status if it was already overdue
        new_status = "overdue" if p.get("status") == "overdue" else "partial"
...
async def vendor_analytics(merchant_id: str):
    ...
    # Calculate spend purely from transactions (single source of truth for expenses)
    try:
...
async def delete_vendor(vendor_id: str):
    ...
    # Prevent deletion if there are outstanding payables
    payables = (
        db.table("payables")
        .select("remaining")
    ...
    if outstanding > 0:
        raise HTTPException(400, f"Cannot delete vendor. You still owe Rs {outstanding:,.2f}.")
