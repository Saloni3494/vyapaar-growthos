...
def _safe_float(val: Any) -> float:
    """Safely convert LLM output to float, stripping commas and currency symbols."""
    if not val:
        return 0.0
    if isinstance(val, (int, float)):
        return float(val)
    try:
        cleaned = str(val).replace(",", "").replace("Rs", "").replace("₹", "").replace("$", "").strip()
...
async def import_from_image(body: OCRImportRequest):
    ...
    if body.extraction_type == "khata":
        entries_raw = data.get("entries", [])
        created_entries = []
        for raw_entry in entries_raw:
            cust_name = (raw_entry.get("customer_name") or "").strip()
            amount = _safe_float(raw_entry.get("amount", 0))
            if not cust_name or amount <= 0:
                continue
                
            new_entry = db.insert("udhari", {
                "merchant_id": body.merchant_id,
            ...
            logger.info("Logged khata udhari entry for %s: %s", cust_name, amount)
    ...
    for raw_item in items_raw:
        ...
        qty = int(_safe_float(raw_item.get("qty", 1)) or 1)
        amount = _safe_float(raw_item.get("amount", 0) or raw_item.get("rate", 0))
        cost_price = round(amount / qty, 2) if qty > 0 else amount
        unit = (raw_item.get("unit") or "pcs").strip()

        # Fuzzy match against existing inventory
        matched = None
        item_lower = item_name.lower().strip()
        for ex in existing_items:
            ex_name = (ex.get("item_name") or "").lower().strip()
            if ex_name == item_lower or item_lower in ex_name or ex_name in item_lower:
                matched = ex
                break

        if matched:
            old_qty = matched.get("current_qty", 0) or 0
            old_cost = matched.get("cost_price", 0) or 0
            new_qty = int(old_qty + qty)
            
            # Weighted Average Cost (WAC)
            if new_qty > 0:
                new_avg_cost = round(((old_qty * old_cost) + amount) / new_qty, 2)
            else:
                new_avg_cost = cost_price

            db.update("inventory", matched["id"], {
                "current_qty": new_qty,
            ...
            db.insert("inventory_adjustments", {
                ...
                "direction": "in",
                "qty": qty,
                "reason": f"OCR Vendor Invoice Import - {vendor}",
            ...
            db.insert("inventory_adjustments", {
                ...
                "direction": "in",
                "qty": qty,
                "reason": f"Initial Stock via OCR - {vendor}",
                "previous_qty": 0,
                "new_qty": int(qty),
                "adjusted_at": datetime.now().isoformat()
            })
    ...
    # Process total for expense tracking
    total_amount = _safe_float(data.get("total", 0) or data.get("subtotal", 0))
    if total_amount > 0:
        ...
        try:
            db.insert("transactions", {
                ...
                "description": f"Purchase Invoice {invoice_number} from {vendor}",
                "recorded_at": datetime.now().isoformat(),
                "source": "ocr_invoice",
            })
            logger.info("Logged expense of %s for OCR invoice %s", total_amount, invoice_number)
        except Exception as e:
            logger.error("Failed to log OCR expense: %s", e)
