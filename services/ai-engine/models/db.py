...
def sync_customer(merchant_id: str, name: str, phone: Optional[str] = None, amount_spent: float = 0.0) -> None:
    """Upsert a customer record to sync CRM stats on new transactions."""
    try:
        ...
        customers = select("customers", filters={"merchant_id": merchant_id, "name": name})
        now_str = datetime.now(timezone.utc).isoformat()
        
        if customers:
            c = customers[0]
            new_visits = int(c.get("total_visits") or 0) + 1
            new_spent = float(c.get("total_spent") or 0.0) + float(amount_spent)
            update_data = {
            ...
            update("customers", c["id"], update_data)
            logger.info(f"Synced existing customer {name} (visits: {new_visits}, spent: {new_spent})")
        else:
            insert("customers", {
        ...
        import logging
        logging.getLogger(__name__).error(f"Failed to sync customer {name}: {e}")
